    ...
    try {
      ...
      function applyResult(result) {
        try {
          ...
          if (result.badge) awardTitleFromEvents(result.badge);
          if (result.loot) {
            window.inventory = window.inventory || [];
            if (!window.inventory.includes(result.loot)) {
              window.inventory.push(result.loot);
            }
            if (typeof delayedLog === "function") {
              delayedLog(`🎒 Loot acquired: ${result.loot}`, "log-positive");
            }
          }
          if (typeof updateScoreboard === "function") updateScoreboard();
      ...
      function finishAndClose() {
        ...
        // Restore normal gameplay music when event tree closes
      ...
      function renderStep(i) {
        ...
        // Switch to Event Story music when the modal is shown
        try { if (typeof window.resumeCurrentMusic === 'function') window.resumeCurrentMusic(); } catch(_) {}
